...
Showing the Apoapsis change of an Earch Orbit after a 100m/s burn.`
)}

function _value(){return(
(x, y) =>
  _apo_after_100ms_accel(_radius("Earth") + x * 1000, _radius("Earth") + y * 1000)
)}
...
function _energy(peri, apo) {
  ...
  const e_k = Math.pow(_vel_at_peri(peri, apo), 2) / 2;
  const e_p = _mu("Earth") / peri;
  return e_k + e_p;
}
...
d3.scaleSequentialLog(d3.extent(thresholds), d3.interpolateYlOrRd)
)}

function _thresholds(d3){return(
d3.range(1, 40).map(i => 0.929e6 / 40 * i)
)}
